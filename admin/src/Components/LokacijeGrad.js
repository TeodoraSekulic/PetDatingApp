...
export default function LokacijeGrad() {
  ...
  const handleObrisi = async (id) => {
    await fetch(api + `deleteLocationCity/` + id, {
      method: "DELETE",
  ...
  const handleDodaj = async (event) => {
    let podaci = { name: naziv, idCountry: drzava.identity };
    await fetch(api + `postLocationCity/`, {
      withCredentials: true,
      method: "POST",
      headers: {
